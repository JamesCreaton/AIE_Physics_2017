...
{
	if (yOffset != 0.0){
		glm::mat4 rot = glm::rotate((float)(m_fRotationSpeed * deltaTime * -yOffset), glm::vec3(1, 0, 0));
	...
	if (xOffset != 0.0){
		glm::mat4 rot = glm::rotate((float)(m_fRotationSpeed * deltaTime * -xOffset), glm::vec3(0, 1, 0));
		SetTransform(GetTransform() * rot);
	}
